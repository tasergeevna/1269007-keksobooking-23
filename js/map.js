...
similarAds.forEach((ad) => {
  ...
  } else {popupElement.querySelector('.popup__title').textContent = ad.offer.title;
  }

  if (!ad.offer.address) {
    popupElement.querySelector('.popup__text--address').remove();
  } else {popupElement.querySelector('.popup__text--address').textContent = ad.offer.address;
  }

  if (!ad.offer.price) {
    popupElement.querySelector('.popup__text--price').remove();
  } else {popupElement.querySelector('.popup__text--price').textContent = `${ad.offer.price} ₽/ночь`;
  }

  if (!ad.offer.type) {
    popupElement.querySelector('.popup__type').remove();
  } else {popupElement.querySelector('.popup__type').textContent = RusTypes[ad.offer.type];
  }

  if (!ad.offer.rooms || !ad.offer.guests) {
    popupElement.querySelector('.popup__text--capacity').remove();
  } else {popupElement.querySelector('.popup__text--capacity').textContent = `${ad.offer.rooms} комнаты для ${ad.offer.guests} гостей`;
  }

  if (!ad.offer.checkin || !ad.offer.checkout) {
    popupElement.querySelector('.popup__text--time').remove();
  } else {popupElement.querySelector('.popup__text--time').textContent = `Заезд после ${ad.offer.checkin}, выезд до ${ad.offer.checkout}`;
  }
  ...
  } else {popupElement.querySelector('.popup__description').textContent = ad.offer.description;
  }
  ...
  popupElement.appendChild(PICS);

  popupElement.querySelector('.popup__avatar').src = ad.author;

  popupElementsList.appendChild(popupElement);
});
